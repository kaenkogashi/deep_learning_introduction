...
def digitize_state(observation):
    ...
    digitized = [
        ...
        np.digitize(pole_angle, bins=bins(-0.5, 0.5, num_dizitized)),#3.14159=180度
...
# 状態を6分割^（4変数）にデジタル変換してQ関数（表）を作成,おまじないだと思う
num_dizitized = 6  #分割数
...
for episode in range(num_episodes):  #試行数分繰り返す
    ...
    state = digitize_state(observation)#観測observationの４つから最大価値Q関数を戻す
    action = np.argmax(q_table[state])#arg maxf(x)：f(x) を最大にする x の集合
    episode_reward = 0
    ...
    for t in range(max_number_of_steps):  #1試行のループ
        ...
        if done:
            if t < 195:
                ...
                reward = 100  #立ったまま終了時は罰則はなし
        else:
            reward = 100  #各ステップで立ってたら報酬追加
